...
def report_progress():
    ...
    while not ProgressBar.exec_finished:
        ...
        percentage_complete = round(
            ProgressBar.current_step / ProgressBar.max_step * 100, 2
        )
        percentage_complete = str(percentage_complete).rjust(5, " ")
        util.print_tr(
            f"{int(current_time)}s: [{ProgressBar.current_stage}] "
            f"{percentage_complete}% complete "
            f"({ProgressBar.current_step} / {ProgressBar.max_step} tasks)"
        )
