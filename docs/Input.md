...
> [!CAUTION] Empty Folders in Tags Out?
> Check your command line arguments!
> - Fuzzy Search only outputs the types of analysis you requested.
> - Make sure you specify at least one type of output.
...
```bash
# Move into the folder with fuzzysearch's .py files, then run the following:
python postdownloader.py
```
...
Fuzzy Search creates samples of all these settings during your first run. Use these pre-made files as a reference.
...
### Blacklist
> [!NOTE] Soft and Hard Blacklisting
> Fuzzy Search uses two blacklists:
> - A "hard", binary/E6-style blacklist which hides any post with a given tag
...
2. Use the `--blacklist <threshold>` argument to apply the soft blacklist when processing searches. This argument is not applied by default. The hard, E6-style blacklist is always applied, whether the soft blacklist is used or not.
