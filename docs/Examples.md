...
Fuzzy Search's customizable `--bar` charts plot the ratio of posts in your search that contain various tags.
...
To get the same data on E6, every single one of these bars represents another search. Fuzzy Search can do the same in a single search, and visualizes all those numbers with a bar chart!
...
>A: "Ooh, feeling daring today aren't we~"

You didn't say who your favorite character is, but let me guess...
...
---
If you want to find artists who have *specialized* in posts with Loona (meaning relatively few non-Loona posts), then `--recommended` can do that! Recommended will try to steer you to tags that 1) have lots of posts with what you searched and 2) have few posts outside of your search.
...
2) that same tag is rarely used on posts outside of what you searched for
...
**Note: recommended is not the same as popular/common.**
...
> Q: "I have a favorite artist, but when I use Fuzzy Search to find recommended tags, the only artist tags that come up are the artist I used in my Search!"
...
Fuzzy Search considers *only* the posts that came up in your search. This helps keep the recommended tags, y'know, *relevant* to what you searched. When you search for an artist tag, you'll typically get 100% of the posts from that artist and zero posts from other artists:
```text
### artist
#set|#db    %set| tag
...
~tags tend to lead to balloon the amount of posts in a search, which can be tiresome to trawl through. Using a command such as `--order "~" --top 20.0` keeps the first 20% of posts (the posts with the most ~tags) while hiding the remaining 80% of posts (the posts with the least ~tags).
...
---
You may be asking:
> Q: **Why don't you add `detailed_background` to filter the posts?** 
> A: Searching for `loona_(helluva_boss) detailed_background` reduces the pool from from about 30,000 to a more manageable 1,000 posts. This saves the effort of making a custom category, but there are some issues with this approach.

 **Issue 1: Under-tagging**
 Someone might tag a post with `day outside grass` but forget to tag `detailed_background`. Despite having other background-related tags (and thus image content that qualifies as a detailed background), this post would be excluded with a search for `loona_(helluva_boss) detailed_background`. This type of issue is a symptom of under-tagging. Given that E6 is a volunteer-run site with millions of posts, under-tagging is pretty common. This issue is especially common on smaller/niche tags, which are the types of tags Fuzzy Search was designed to help discover.
...
- Adding `night ~day ~indoors` and other ~tags ultimately leads right back to the original list of 1,000 posts.
	...
	- If you're doing this on Fuzzy Search, you can at least order posts by number of ~tags (--order: `~`)... which is actually a custom category under the hood. So you might as well bite the bullet and make a proper "Detailed Background" custom category filled with the ~tags you used.
...
If you're unfamiliar with reading percentile charts, they're trying to visualize what percentage of posts have some attribute (in this example it's the number of tags in the "Detailed Background" custom category).
...
>A: "It's `current_year`, so it's high time for... the non-binary blacklist!"
...
halloween -1.0
```
Note: The default weight is 1.0, so the below custom category is identical to the above:
```
skull 1.0
bones 1.0
skeleton 1.0
...
> A: "To help inform your decision, run Fuzzy Search with `-all` and skim the variety of outputs."
...
Here's the results of applying Fuzzy Search's soft blacklist to different artists. Rather than hiding high-relevancy blacklist posts (`--blacklist`), we generate the below charts with `--charts_custom`. This is possible because the soft blacklist is just a custom category with a special name.
![Chart comparing blacklist percentiles for two artists.](/images/fs/compare_one.png)

**Reading the chart legend:** "Blacklist 0.59 (397)" states the custom category name, the average custom category relevancy per post, and total relevancy across all posts.
...
**Notes:**
`--graph 500` tells Fuzzy Search to generate graphs for searches with 500 or less posts. For performance reasons, try to limit Obsidian graphs to searches with no more than 1,000 posts. See the [Input](docs/Input.md) page for how to set up an Obsidian Vault from Fuzzy Search's output.
